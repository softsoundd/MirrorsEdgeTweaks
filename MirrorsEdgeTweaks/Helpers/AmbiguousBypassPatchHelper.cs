...
{
    public enum AmbiguousBypassPatchState
    {
        Unknown,
        Unpatched,
        Patched
    ...
    public static class AmbiguousBypassPatchHelper
    {
        private const uint ImageBase = 0x00400000;
        private const string PatchKey = "ambiguousPackage_bypass";

        public static AmbiguousBypassPatchState GetPatchState(string exePath)
        {
            byte[] data = File.ReadAllBytes(exePath);
            string? version = ExeVersionDetector.DetectVersion(data, exePath);
            if (version == null) return AmbiguousBypassPatchState.Unknown;

            if (version == "ea")
            {
                try { PatchUtility.DecryptOoaInPlace(data); }
                catch { return AmbiguousBypassPatchState.Unknown; }
            }
            ...
            if (!addrs.InlinePatches.TryGetValue(PatchKey, out var patch))
                return AmbiguousBypassPatchState.Unknown;
